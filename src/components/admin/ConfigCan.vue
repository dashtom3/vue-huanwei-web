  ...
  <div class="park">
    ...
    <div class="park-table">
      <el-table
        ...
        <el-table-column prop="speed" label="关联垃圾桶信息">
          <template slot-scope="scope" v-if="scope.row.can[0]">{{scope.row.can[0].name}}</template>
        </el-table-column>
        <el-table-column
          label="操作">
          <template slot-scope="scope">
            <el-button type="text" size="mini" @click="updateCanDialog(scope.row)">设置</el-button>
          </template>
        </el-table-column>
      </el-table>
    </div>
    <el-dialog
      ...
      <el-form ref="form" :model="canEntity" label-width="80px">
        <el-form-item label="gps手机卡号">
          <el-input v-model="canEntity.gps_card"></el-input>
        </el-form-item>
      </el-form>
...
export default {
  ...
  methods:{
    getCanList(_id){
      this.$global.httpGetWithToken(this,'can/allOfConfig').then(res=>{
        console.log(res)
    ...
    updateCan(){
      this.$global.httpPostWithToken(this,'can/operateDevice',{action:1,data:{_id:this.canEntity._id,gps_card:this.canEntity.gps_card}}).then(res=>{
        this.isUpdateShow = !this.isUpdateShow
        this.getCanList()
      })
    }
